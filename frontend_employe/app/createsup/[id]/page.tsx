...
import { hasRole, isAutenticated } from "@/app/models/oauth/authenticated";
...
function Update({ params }: ParamsId) {
    ...
    const [isT, setIsT] = useState<boolean>(false);
    ...
    useEffect(
        () => {
            if (isAutenticated()) {
                ...
                setRoles(hasRole());
                if (!(hasRole().length > 0 && hasRole().includes('ROLE_ADMIN'))) {
                    ruta.push('/main')
                }else{
                    setIsT(true);
                    data();
